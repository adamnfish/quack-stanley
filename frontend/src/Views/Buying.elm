...
import Html exposing (Html, a, br, button, div, h2, li, p, span, text, ul)
import Html.Attributes exposing (class, disabled, href, placeholder)
import Html.Events exposing (onClick, onInput, onSubmit)
import Model exposing (Lifecycle(..), Model, PlayerSummary)
import Msg exposing (Msg)
import Views.Utils exposing (ShroudContent(..), card, col, container, empty, gameNav, helpText, icon, row)
...
buying role awardingTo model =
    let
        ...
        products =
            Maybe.withDefault Dict.empty (Maybe.map .products model.round)
    in
    ( [ button
...
otherPlayers opponents products role =
    if List.isEmpty opponents then
        div
            []
            [ helpText
                "There are no other players in this game so there's no one to award the point to."
            , p []
                [ button
                    [ class "waves-effect waves-light btn blue"
                    , onClick Msg.RelinquishBuyer
...
otherPlayer role products playerSummary =
    let
        hasPitched =
            Dict.member playerSummary.screenName products

        productWords =
            Maybe.withDefault [] (Dict.get playerSummary.screenName products)
