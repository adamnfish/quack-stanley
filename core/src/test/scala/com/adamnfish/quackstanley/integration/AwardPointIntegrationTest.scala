...
class AwardPointIntegrationTest
    extends AnyFreeSpec
    with Matchers
  ...
  "awardPoint" - {
    "if the game exists" - {
      ...
      "and this player and winner are both registered" - {
        ...
        val winningPlayerState =
          PlayerState(gameId, gameName, winnerScreenName, Nil, Nil, None, Nil)
        ...
        "returns player info without role" in {
          val request =
            AwardPoint(gameId, playerKey, Role("role"), winnerScreenName)
        ...
        "returns state includes new point in winners summary" in {
          ...
          val playerInfo = awardPoint(request, testConfig).run()
          val winnerSummary =
            playerInfo.opponents.find(_.screenName == winnerScreenName).value
        ...
        "persists point in winner's state" in {
          val request =
            AwardPoint(gameId, playerKey, Role("role"), winnerScreenName)
          val playerInfo = awardPoint(request, testConfig).run()
          val persistedWinnerState =
            GameIO.getPlayerState(winningPlayerKey, gameId, persistence).run()
        ...
        "persists point in winner's player summary in game's state" in {
          ...
          persistedGameState.players
            .get(winningPlayerKey)
            .value
            .points should contain(Role("role"))
        }
        ...
        "fails if the player is not the buyer" in {
          val request =
            AwardPoint(gameId, creator, Role("role"), winnerScreenName)
          awardPoint(request, testConfig).isFailedAttempt()
        }
        ...
        "validates user input," - {
          ...
          "flags empty role" in {
            val request =
              AwardPoint(gameId, playerKey, Role(""), winnerScreenName)
            val failure = awardPoint(request, testConfig).leftValue()
