...
object QuackStanley {
  ...
  def createGame(data: CreateGame, config: Config): Attempt[NewGame] = {
    ...
    val playerState =
      newPlayer(gameState.gameId, gameState.gameName, data.screenName)
    for {
      _ <- validate(data)
      code <- makeUniquePrefix(
        gameState.gameId,
        config.persistence,
        checkPrefixUnique
      )
  ...
  /** Adds a player with the provided screen name to the specified game.
    *
    * Note that this function does not update the game state to prevent race
    * conditions, this will be done once by the creator when the game is
    * started.
    *
    * Because player info is only added to the game state when the game starts,
    * we need to lookup all the players' info separately to check if the screen
    * name is unique.
    */
  def registerPlayer(
      data: RegisterPlayer,
  ...
  /** Sets this game's state to "started" and sets up the initial player states
    * (e.g. dealing them words).
    *
    * This is also the chance to write the player names/keys into the game
    * state. Doing it from here prevents race hazards since reading and writing
    * S3 files is not atomic.
    */
  def startGame(data: StartGame, config: Config): Attempt[PlayerInfo] = {
    for {
      ...
      words <- nextWords(
        handSize * playerStates.size,
        config.wordSource.words,
        Set.empty
      )
  ...
  /** Signals that the player would like to be the next "buyer". We can deal
    * them a role and wait for people to pitch to that player.
    */
  def becomeBuyer(data: BecomeBuyer, config: Config): Attempt[PlayerInfo] = {
    for {
      ...
      role <- nextRole(
        config.wordSource.roles,
        usedRoles(players.values.toList)
      )
      playerWithRole = player.copy(role = Some(role))
      gameWithBuyer = gameState.copy(round =
        Some(Round(data.playerKey, role, Map.empty))
      )
  ...
  /** Allows player to stop being the game's "buyer" without granting a point to
    * another player.
    */
  def relinquishBuyer(
      data: RelinquishBuyer,
      config: Config
  ): Attempt[PlayerInfo] = {
    for {
      _ <- validate(data)
  ...
  def startPitch(data: StartPitch, config: Config): Attempt[PlayerInfo] = {
    ...
    EitherT.leftT(
      Failure(
        "start pitch is not implemented",
        "'start pitch' functionality has not yet been created",
        404
      ).asFailedAttempt
    )
  }
  ...
  def finishPitch(data: FinishPitch, config: Config): Attempt[PlayerInfo] = {
    for {
      ...
      gameStateWithPitch <- updateGameWithPitch(
        gameState,
        data.playerKey,
        data.words
      )
  ...
  def awardPoint(data: AwardPoint, config: Config): Attempt[PlayerInfo] = {
    for {
      ...
      (winningPlayerKey, winningPlayer) <- lookupPlayerByName(
        players,
        data.awardToPlayerWithName
      )
      updatedGameState <- updateGameWithAwardedPoint(
        gameState,
        winningPlayerKey,
        data.role
      )
  ...
  /** Player discards a point (if they have one) and their current hand. Player
    * is given a new hand of words.
    */
  def mulligan(data: Mulligan, config: Config): Attempt[PlayerInfo] = {
    ...
    EitherT.leftT(
      Failure(
        ...
        "'mulligan' functionality has not yet been created",
        404
      ).asFailedAttempt
    )
  }
  ...
  /** return current state for the player.
    *
    * Ping is called very regularly so optimisations are in place to reduce
    * costs.
    */
  ...
  /** return current state for the player with a description of who has joined
    * the game.
    *
    * LobbyPing is called regularly so optimisations are in place to reduce
    * costs.
    */
