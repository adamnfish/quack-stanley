...
object Serialization {
  ...
  implicit val dateTimeEncoder: Encoder[DateTime] =
    Encoder.encodeString.contramap[DateTime](dtfmt.print)
  ...
  // value classes
  implicit val playerKeyEncoder: Encoder[PlayerKey] =
    Encoder.encodeString.contramap[PlayerKey](_.value)
  implicit val playerKeyDecoder: Decoder[PlayerKey] =
    Decoder.decodeString.emap(str => Right(PlayerKey(str)))
  implicit val playerKeyAsMapKeyEncoder: KeyEncoder[PlayerKey] =
    (pk: PlayerKey) => pk.value
  implicit val playerKeyAsMapKeyDecoder: KeyDecoder[PlayerKey] =
    (pkStr: String) => Some(PlayerKey(pkStr))
  implicit val gameIdEncoder: Encoder[GameId] =
    Encoder.encodeString.contramap[GameId](_.value)
  implicit val gameIdDecoder: Decoder[GameId] =
    Decoder.decodeString.emap(str => Right(GameId(str)))
  implicit val roleEncoder: Encoder[Role] =
    Encoder.encodeString.contramap[Role](_.value)
  implicit val roleDecoder: Decoder[Role] =
    Decoder.decodeString.emap(str => Right(Role(str)))
  implicit val wordEncoder: Encoder[Word] =
    Encoder.encodeString.contramap[Word](_.value)
  implicit val wordDecoder: Decoder[Word] =
    Decoder.decodeString.emap(str => Right(Word(str)))

  // inputs
  implicit val createGameDecoder: Decoder[CreateGame] =
    deriveDecoder[CreateGame]
  implicit val registerPlayerDecoder: Decoder[RegisterPlayer] =
    deriveDecoder[RegisterPlayer]
  implicit val startGameDecoder: Decoder[StartGame] = deriveDecoder[StartGame]
  implicit val becomeBuyerDecoder: Decoder[BecomeBuyer] =
    deriveDecoder[BecomeBuyer]
  implicit val relinquishBuyerDecoder: Decoder[RelinquishBuyer] =
    deriveDecoder[RelinquishBuyer]
  implicit val startPitchDecoder: Decoder[StartPitch] =
    deriveDecoder[StartPitch]
  implicit val finishPitchDecoder: Decoder[FinishPitch] =
    deriveDecoder[FinishPitch]
  implicit val awardPointDecoder: Decoder[AwardPoint] =
    deriveDecoder[AwardPoint]
  ...
  implicit val apiOperationDecoder: Decoder[ApiOperation] =
    Decoder.instance(c =>
      c.downField("operation").as[String].flatMap {
        ...
        case "lobbyPing"        =>
          c.as[LobbyPing] // compatibility for now TODO: remove this
        case "wake" => c.as[Wake]
      }
    )

  // response types
  implicit val playerInfoEncoder: Encoder[PlayerInfo] =
    deriveEncoder[PlayerInfo]
  ...
  implicit val registeredEncoder: Encoder[Registered] =
    deriveEncoder[Registered]
  ...
  implicit val opponentEncoder: Encoder[PlayerSummary] =
    deriveEncoder[PlayerSummary]
  implicit val opponentDecoder: Decoder[PlayerSummary] =
    deriveDecoder[PlayerSummary]

  implicit val playerStateEncoder: Encoder[PlayerState] =
    deriveEncoder[PlayerState]
  implicit val playerStateDecoder: Decoder[PlayerState] =
    deriveDecoder[PlayerState]
