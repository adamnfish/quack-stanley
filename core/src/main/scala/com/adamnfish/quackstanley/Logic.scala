...
object Logic {
  ...
  def lobbyPlayerInfo(
      playerKey: PlayerKey,
      playerState: PlayerState,
      playerStates: Map[PlayerKey, PlayerState]
  ): PlayerInfo = {
    val otherPlayers = playerSummaries(playerStates).filterNot {
      case (opponentKey, _) =>
  ...
  def authenticate(
      playerKey: PlayerKey,
      gameState: GameState
  ): Attempt[PlayerSummary] = {
    EitherT.fromOption[IO](
      gameState.players.get(playerKey),
      Failure(
        "Player key not found in game state",
  ...
  def authenticateCreator(
    ...
    if (gameState.creator == playerKey) {
      ...
      EitherT.leftT {
        Failure(
          "Player key not found in game state",
          "You are not the game's creator",
          404
        ).asFailedAttempt
      }
    }
  ...
  def authenticateBuyer(
    ...
    EitherT.fromOption[IO](
      ...
      Failure(
        "Player is not buyer",
        "Another player is already the buyer",
        404
      ).asFailedAttempt
    )
  }
  ...
  def playerHasRole(playerState: PlayerState, role: Role): Attempt[Role] = {
    EitherT.fromOption[IO](
      playerState.role.find(_ == role),
      Failure(
        "Player does not have this role to give",
        "Cannot award point for a role that isn't yours",
        400
      ).asFailedAttempt
    )
  }
  ...
  def playerSummaries(
      states: Map[PlayerKey, PlayerState]
  ): Map[PlayerKey, PlayerSummary] = {
  ...
  def lookupPlayer(
    ...
    EitherT.fromOption[IO](
      states.get(playerKey),
      Failure(
        "Couldn't lookup creator's state",
  ...
  def lookupPlayerByName(
      states: Map[PlayerKey, PlayerState],
      screenName: String
  ): Attempt[(PlayerKey, PlayerState)] = {
    EitherT.fromOption[IO](
      states.find(_._2.screenName == screenName),
      Failure(
        "Couldn't lookup player by name",
        s"Couldn't find a player called '$screenName'",
        500,
        None
      ).asFailedAttempt
    )
  ...
  def startGameState(
      gameState: GameState,
      playerNames: Map[PlayerKey, PlayerSummary]
  ): GameState = {
    gameState.copy(
      started = true,
      players = playerNames
  ...
  def verifyNoBuyer(gameState: GameState): Attempt[Unit] = {
    gameState.round match {
      case None =>
        EitherT.pure(())
      case Some(buyer) =>
        val playerName = gameState.players.view
          .mapValues(_.screenName)
          .toMap
          .getOrElse(buyer.buyerKey, "another player")
        EitherT.leftT(
          Failure(
            s"Buyer already exists: $playerName",
            s"$playerName is already the buyer",
            400
          ).asFailedAttempt
        )
    }
  ...
  def verifyUniqueScreenName(
      screenName: String,
      playerStates: Map[PlayerKey, PlayerState]
  ): Attempt[Unit] = {
    playerStates
      .find { case (_, playerState) =>
  ...
  def updateGameWithPitch(
    ...
    EitherT.fromOption[IO](
      ...
      FailedAttempt(
        Failure(
          "Cannot add pitch to game, no round in progress",
          "There's no buyer to pitch to",
          500
        )
      )
    )
  ...
  def updateGameWithAwardedPoint(
      gameState: GameState,
      winner: PlayerKey,
      role: Role
  ): Attempt[GameState] = {
    for {
      winnerSummary <- EitherT.fromOption[IO](
        gameState.players.get(winner),
        FailedAttempt(
          Failure(
            "Could not find winner in game state's players",
            "Could not find player",
            404
          )
        )
      )
      updatedWinnerSummary = winnerSummary.copy(points =
        winnerSummary.points :+ role
      )
  ...
  def nextRole(roles: List[Role], used: Set[Role]): Attempt[Role] = {
    EitherT.fromOption[IO](
      shuffle(roles).filterNot(used.contains).headOption,
      Failure(
        "Exhausted available roles",
        "Ran out of roles",
        500
      ).asFailedAttempt
    )
  ...
  def dealWordsToAllPlayers(
      words: List[Word],
      players: Map[PlayerKey, PlayerState]
  ): Attempt[Map[PlayerKey, PlayerState]] = {
    if (words.size < players.size * QuackStanley.handSize) {
      EitherT.leftT(
        Failure(
          "dealWords wasn't given enough words for the players",
          "Failed to get words for all players",
          500
        ).asFailedAttempt
      )
    } else {
      EitherT.pure {
        val (_, dealtPlayers) =
          players.foldLeft[(List[Word], Map[PlayerKey, PlayerState])](
            (words, Map.empty)
          ) { case ((remainingWords, acc), (playerKey, playerState)) =>
            (
              remainingWords.drop(QuackStanley.handSize),
              acc + (playerKey -> playerState.copy(hand =
                remainingWords.take(QuackStanley.handSize)
              ))
            )
          }
        dealtPlayers
  ...
  def fillHand(
    ...
    if (words.size < (QuackStanley.handSize - playerState.hand.size)) {
      EitherT.leftT(
        Failure(
          "Not enough words provided to fill player hand",
          "Ran out of words",
          500
        ).asFailedAttempt
      )
  ...
  def discardWords(
    ...
    val failures = List(words._1, words._2).flatMap { word =>
      if (playerState.hand.contains(word)) None
        ...
        Some(
          Failure(
            "Player cannot discard word not in their hand",
            s"Cannot discard words that aren't in your hand ($word)",
            400,
            Some(word.value)
          )
        )
    ...
    def loop(prefixLength: Int): Attempt[String] = {
      fn(gameId, prefixLength, persistence).flatMap {
        ...
        case _ =>
          EitherT.leftT(
            Failure(
              "Couldn't create unique prefix of GameID",
              "Couldn't set up game with a join code",
              500
            ).asFailedAttempt
          )
      }
  ...
  def gameIdFromPrefixResults(
    ...
    matches match {
      case Nil =>
        EitherT.leftT(
          Failure(
            "Couldn't find game from gameCode",
      ...
      case _             =>
        EitherT.leftT(
          Failure(
            "Multiple games matched gameCode",
            "Couldn't find a game to add you to, invalid code",
            404,
            Some(gameCode)
          ).asFailedAttempt
        )
